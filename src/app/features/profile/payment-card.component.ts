/*
 * PaymentCardComponent: Editable card for payment methods.
 * Uses reactive form, subscribes to service.
 * Note: For simplicity, editing adds/removes methods; in real app, use child forms for each method.
 */
    ...
    <div class="bg-card p-6 rounded-lg shadow-md">
      ...
      @if (editMode()) {
        <form [formGroup]="form" (ngSubmit)="save()">
            ...
            @for (method of paymentMethods.controls; track $index) {
              <div class="grid grid-cols-2 gap-4 mb-4" [formGroupName]="$index">
                <div>
                  ...
                  <input formControlName="type" class="w-full p-2 border rounded" />
                </div>
                <div>
          ...
          <button type="button" (click)="addMethod()" class="mb-4 px-4 py-2 bg-gray-300 rounded">Add Method</button>
